-- ============================================================
-- SCHEMA ABSENSI KARYAWAN — Versi Supabase Lama
-- Gurus table menggunakan kolom EMAIL (bukan auth_user_id)
-- Jalankan di SQL Editor Supabase lama
-- ============================================================
...
-- ============================================================
-- LANGKAH 2 — Helper functions (lookup via email)
-- ============================================================
...
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE g.email = (SELECT email FROM auth.users WHERE id = auth.uid() LIMIT 1)
    ),
    '{}'::TEXT[]
  );
$$;
...
-- ============================================================
-- LANGKAH 3 — RLS untuk guru_units
-- ============================================================
...
CREATE POLICY "shifts_admin_insert" ON shifts FOR INSERT WITH CHECK (absensi_is_admin());
CREATE POLICY "shifts_admin_update" ON shifts FOR UPDATE USING (absensi_is_admin()) WITH CHECK (absensi_is_admin());
CREATE POLICY "shifts_admin_delete" ON shifts FOR DELETE USING (absensi_is_admin());
...
CREATE POLICY "ss_select"       ON shift_schedules FOR SELECT USING (true);
CREATE POLICY "ss_admin_insert" ON shift_schedules FOR INSERT WITH CHECK (absensi_is_admin());
CREATE POLICY "ss_admin_update" ON shift_schedules FOR UPDATE USING (absensi_is_admin());
CREATE POLICY "ss_admin_delete" ON shift_schedules FOR DELETE USING (absensi_is_admin());
...
CREATE POLICY "att_update" ON attendances FOR UPDATE
  USING (guru_id = absensi_guru_id() OR (absensi_is_admin() AND unit_id = ANY(absensi_unit_ids())));
...
CREATE POLICY "lr_select"       ON leave_requests FOR SELECT USING (guru_id = absensi_guru_id() OR absensi_is_admin());
CREATE POLICY "lr_insert_self"  ON leave_requests FOR INSERT WITH CHECK (guru_id = absensi_guru_id());
CREATE POLICY "lr_update_admin" ON leave_requests FOR UPDATE USING (absensi_is_admin());
...
CREATE POLICY "ot_select"      ON overtime FOR SELECT USING (guru_id = absensi_guru_id() OR absensi_is_admin());
CREATE POLICY "ot_insert_self" ON overtime FOR INSERT WITH CHECK (guru_id = absensi_guru_id());
CREATE POLICY "ot_update"      ON overtime FOR UPDATE USING (guru_id = absensi_guru_id() OR absensi_is_admin());
...
CREATE POLICY "ac_select"       ON attendance_corrections FOR SELECT USING (guru_id = absensi_guru_id() OR absensi_is_admin());
CREATE POLICY "ac_insert_self"  ON attendance_corrections FOR INSERT WITH CHECK (guru_id = absensi_guru_id());
CREATE POLICY "ac_update_admin" ON attendance_corrections FOR UPDATE USING (absensi_is_admin());
...
CREATE POLICY "hl_select"       ON hari_libur FOR SELECT USING (true);
CREATE POLICY "hl_admin_insert" ON hari_libur FOR INSERT WITH CHECK (absensi_is_admin());
CREATE POLICY "hl_admin_update" ON hari_libur FOR UPDATE USING (absensi_is_admin());
CREATE POLICY "hl_admin_delete" ON hari_libur FOR DELETE USING (absensi_is_admin());
